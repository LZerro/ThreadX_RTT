...
UINT  _tx_timer_change(TX_TIMER *timer_ptr, ULONG initial_ticks, ULONG reschedule_ticks)
{
    ...
    {
        rt_timer_control(timer_ptr, RT_TIMER_CTRL_SET_TIME, initial_ticks);
        if(reschedule_ticks == 0)
        {
            timer_ptr->TIME_START_FLAG == TX_TIMER_ONLY_ONCE;
            rt_timer_control((rt_timer_t)timer_ptr, RT_TIMER_CTRL_SET_ONESHOT);
        }
        else
        {
            timer_ptr->TIME_START_FLAG == TX_TIMER_NEED_RESTART;
            timer_ptr->tx_timer_internal_re_initialize_ticks = reschedule_ticks;
            rt_timer_control((rt_timer_t)timer_ptr, RT_TIMER_CTRL_SET_PERIODIC);
        }
    }
}